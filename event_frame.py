...
class EventFrame(tk.Frame):
    def __init__(self, master, title, target_date, update_callback,
        ...
        self.create_widgets()
        self.update_dday()  # 초기 D-Day 표시

        # 더블 클릭 시 제목/날짜 수정
        self.canvas.bind("<Double-1>", self.modify)
        # 오른쪽 클릭 시 컨텍스트 메뉴(추가/삭제) 표시
        self.canvas.bind("<Button-3>", self.show_context_menu)

    def create_widgets(self):
        # 둥근 모서리의 흰색 사각형(카드) 그리기
        self.create_rounded_rectangle(0, 0, self.width, self.height, radius=20, fill="white", outline="")
        # 제목 텍스트 (왼쪽 정렬)
        self.title_text_id = self.canvas.create_text(10, 25, text=self.title, anchor='w',
                                                     font=("맑은 고딕", 12, "bold"), fill="black")
        # 날짜 및 시간 텍스트 (왼쪽 정렬)
        self.date_text_id = self.canvas.create_text(10, 50, text=get_date_string(self.target_date), anchor='w',
                                                    font=("맑은 고딕", 10), fill="black")
        # D-Day 텍스트 (가운데 정렬)
        self.dday_text_id = self.canvas.create_text(self.width / 2, 80, text="",
    ...
    def update_dday(self):
        ...
        self.remaining_seconds = diff.total_seconds()  # 정렬 등에 사용
    ...
    def modify(self, event):
        ...
        if new_date_str:
            try:
                ...
                print("잘못된 날짜 형식입니다. 예시 형식: 2025-02-25 08:00")
...
def get_date_string(target):
    """
    target datetime을 "YYYY-MM-DD (요일) 오전/오후 HH:MM" 형태의 문자열로 반환
    """
